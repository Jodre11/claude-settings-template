...
// Helper: Validate that a file path stays within the allowed base directory
...
// Helper: Sanitize a file:// or absolute path for image sources
function sanitizeImagePath(src, baseDir) {
  let imagePath = src.startsWith('file://') ? src.replace('file://', '') : src;
  imagePath = decodeURIComponent(imagePath);
  return validateFilePath(imagePath, baseDir);
}

// Helper: Add background to slide
async function addBackground(slideData, targetSlide, tmpDir, baseDir) {
  if (slideData.background.type === 'image' && slideData.background.path) {
    const imagePath = sanitizeImagePath(slideData.background.path, baseDir);
...
function addElements(slideData, targetSlide, pres, baseDir) {
  for (const el of slideData.elements) {
    if (el.type === 'image') {
      const imagePath = sanitizeImagePath(el.src, baseDir);
...
async function html2pptx(htmlFile, pres, options = {}) {
  ...
  try {
    ...
    addElements(slideData, targetSlide, pres, baseDir);
