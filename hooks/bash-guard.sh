...
if echo "$cmd" | grep -qE '(/tmp/|\$TMPDIR|/var/folders/)'; then
    ...
    if echo "$cmd" | grep -qE '(\$TMPDIR|/var/folders/)'; then
        msg="TEMP DIRECTORY VIOLATION: Use /tmp/claude-{session_name}/ instead of \$TMPDIR or /var/folders/. See CLAUDE.md 'Temporary Files' section."
        jq -n --arg m "$msg" '{
            "hookSpecificOutput": {
    ...
    msg="TEMP DIRECTORY VIOLATION: Use /tmp/claude-{session_name}/ for writing to temp. See CLAUDE.md 'Temporary Files' section."
    jq -n --arg m "$msg" '{
        "hookSpecificOutput": {
            "hookEventName": "PreToolUse",
            "permissionDecision": "deny",
