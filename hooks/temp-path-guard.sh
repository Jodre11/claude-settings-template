...
if echo "$file_path" | grep -qE '^(/tmp/|/var/folders/)'; then
    msg="TEMP DIRECTORY VIOLATION: Attempted to write to '$file_path'. Per CLAUDE.md 'Temporary Files': use /tmp/claude-\$PPID/ for ALL temp files. Create with 'mkdir -p /tmp/claude-\$PPID' before first use. NEVER use bare /tmp/, /var/folders/, or \$TMPDIR."
    jq -n --arg m "$msg" '{
        "hookSpecificOutput": {
