...
hydrate_settings_json() {
    ...
    #   - hooks: template wins (infrastructure, edited via template workflow)
    #   - enabledPlugins, extraKnownMarketplaces: existing wins (personal config)
    #   - enabledMcpjsonServers: union (template + existing, deduplicated)
    ...
    merged=$(jq -s '
        ...
        | .skillListingBudgetFraction = ($existing.skillListingBudgetFraction // $tmpl.skillListingBudgetFraction)
        ...
        | .enabledMcpjsonServers = (
            ($existing.enabledMcpjsonServers // []) + (
                ($tmpl.enabledMcpjsonServers // []) - ($existing.enabledMcpjsonServers // [])
            )
        )

        # enabledPlugins: existing wins (personal config; do not override fork preferences)
        | .enabledPlugins = ($existing.enabledPlugins // $tmpl.enabledPlugins)

        # extraKnownMarketplaces: existing wins (personal config)
        | .extraKnownMarketplaces = ($existing.extraKnownMarketplaces // $tmpl.extraKnownMarketplaces)

        # hooks: template wins (infrastructure, not local config)
        | .hooks = ($tmpl.hooks // $existing.hooks)

        # statusLine: existing wins
        | .statusLine = ($existing.statusLine // $tmpl.statusLine)

        # Drop null-valued keys so they do not pollute the output
